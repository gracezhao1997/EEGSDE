...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def sample_p_zs_given_zt(self, s, t, zt, node_mask, edge_mask, context, fix_noise=False, guidance=None, l=None,
        ...
        if l2 > 0:
            c2 = context[:, 1, 1]
            with RequiresGradContext(zt, requires_grad=True):
                prediction = guidance2.phi(zt, t, node_mask, edge_mask)
                energy = l2_square(prediction.reshape(bs, -1), c2.reshape(bs, -1))
                grad = autograd.grad(energy.sum(), zt)[0]
    ...
    def sample(self, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise=False, guidance=None, l=None,
        ...
        for s in reversed(range(0, self.T)):
            ...
            z = self.sample_p_zs_given_zt(s_array, t_array, z, node_mask, edge_mask, context, fix_noise=fix_noise,
                                          guidance=guidance, l=l, guidance2=guidance2, l2=l2)
