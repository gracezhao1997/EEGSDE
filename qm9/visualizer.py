...
def save_xyz_file(path, one_hot, charges, positions, dataset_info, id_from=0, name='molecule', node_mask=None):
    ...
    for batch_i in range(one_hot.size(0)):
        f = open(os.path.join(path,name + '_' + "%03d.txt" % (batch_i + id_from)) , "w")
        f.write("%d\n\n" % atomsxmol[batch_i])
...
def visualize_chain(data_path,save_path, dataset_info, spheres_3d=False,):
    files = load_xyz_files(data_path)
    files = sorted(files)
    save_paths = []
    ...
    for i in range(len(files)):
        ...
        names = file.split('/')[-1]
        name = names.split('.')[0] + '.png'
        fn = os.path.join(save_path,name)
        atom_type = torch.argmax(one_hot, dim=1).numpy()
