...
#                 molecule.
#         """
#         # a = self.out_net(fingerprints)#(Bs,128)
#         # b = a[:, None, :]#(Bs,1,128)
#         return self.out_net(fingerprints)#(Bs,128)
...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=3, attention=False,
        ...
        self.fingerprintembedding = nn.Sequential(
            nn.Linear(1024, 768),
            nn.SiLU(),
            nn.Linear(768, 512),
            nn.SiLU(),
            nn.Linear(512, self.hidden_nf),
        )
    ...
    def forward(self, h, x, edge_index, node_mask=None, edge_mask=None,context=None):
        ...
        if self.sin_embedding is not None:
            distances = self.sin_embedding(distances)
        h = self.embedding(h)
        context = self.fingerprintembedding(context)
        h = h + context
        for i in range(0, self.n_layers):
