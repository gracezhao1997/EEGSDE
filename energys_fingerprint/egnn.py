...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=3, attention=False,
        ...
        self.graph_dec = nn.Sequential(nn.Linear(self.hidden_nf, 1024),
                                       act_fn,
                                       nn.Linear(1024, 1024))
        self.to(self.device)
    ...
    def forward(self, h, x, edge_index, node_mask=None, edge_mask=None,n_nodes=None):
        ...
        if self.sin_embedding is not None:
            distances = self.sin_embedding(distances)
        # h:(Bs*nodes,6),x:(Bs*nodes,3)
        h = self.embedding(h)  # (Bs*nodes,192)
        for i in range(0, self.n_layers):
            h, _ = self._modules["e_block_%d" % i](h, x, edge_index, node_mask=node_mask, edge_mask=edge_mask,edge_attr=distances)
        # (Bs*nodes,192)
        h = self.node_dec(h)  # (Bs*nodes,192)
        h = h * node_mask
        h = h.view(-1, n_nodes, self.hidden_nf)  # (Bs,nodes,192)
        h = torch.sum(h, dim=1)  # (Bs,192)
        pred = self.graph_dec(h)  # (Bs,1024)
